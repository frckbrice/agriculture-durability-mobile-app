...
export default function SaleSlipDocumentUpload() {
    ...
    const handlePhotoUpload = async () => {
        ...
        try {
            ...
            if (!result.canceled) {
                ...
                // get the upload url from bucket s3
                const uploadUrl = await uploadToS3(uri, `SEV_${marketData?.market_number}-${uri.split('/').pop()}`, marketData?.company_bucket ?? '', 'application/pdf');
    ...
    const uploadDocuments = useCallback(async () => {
        ...
        try {
            ...
            const objToSend = {
                bon_entree_magazin_url: pendingDocs[0].uri
            };
                ...
                <View className="flex-1 bg-black/50">
                    ...
                    <View className="flex-1 justify-center p-5">
                        <View className="flex-col justify-between h-2/3">
                            ...
                            >
                                <Text className="text-white text-center font-semibold">Back to  Market</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
